...
```bash
campaign-segmentation-workflow$ sam build --use-container
```
...
```bash
campaign-segmentation-workflow$ sam logs -n StockCheckerFunction --stack-name 
campaign-segmentation-workflow --tail
```
...
```bash
campaign-segmentation-workflow$ pip install -r tests/requirements.txt --user
# unit test
campaign-segmentation-workflow$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
campaign-segmentation-workflow$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name campaign-segmentation-workflow
```
